    <div class='container div-principal formulario'>
        ...
        <?php if (!(isset($dados['dado']['idtb_multa']) == NULL)) : ?>
            <form class="container text-center" action="<?= URL . "/multas/edit/" . $dados['dado']['idtb_multa'] ?>" method="POST">
                <h3> Editar <?= $dados['dado']['idtb_multa'] ?></h3>
            <?php else : ?>
                <form class="container text-center" action=<?= URL . "/multas/insert" ?> method="POST">
                    <h3>Multa</h3>
                <?php endif ?>

                <select name='ano' class="select form-select form-select-lg mb-3 <?= $dados['erro']['ano'] ? 'is-invalid' : '' ?>" aria-label=".form-select-lg example">
                ...
                <div class="form-floating">
                    <input value="<?= $dados['dado']['cidade'] ?>" name="cidade" class="form-control <?= $dados['erro']['cidade'] ? 'is-invalid' : 'input' ?>" placeholder="Cidade">
                ...
                <select name='idtb_carro' class="select form-select form-select-lg mb-3 <?= $dados['erro']['idtb_carro'] ? 'is-invalid' : '' ?>" aria-label=".form-select-lg example">
                ...
                <select name='idtb_infracao' class="select form-select form-select-lg mb-3 <?= $dados['erro']['idtb_infracao'] ? 'is-invalid' : '' ?>" aria-label=".form-select-lg example">
